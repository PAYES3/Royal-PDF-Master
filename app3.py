...
# --- 🚀 FORCED AD TRIGGER (Background Method) ---
def trigger_ads():
    # Box layout-ah nambaama, background-la script-a run panna vaikka
    # Indha script mobile-la social bar-a kandippa trigger pannum
    ad_script = """
    ...
    <div style="background-color: #f1f1f1; border: 2px dashed #007bff; border-radius: 10px; padding: 20px; text-align: center;">
        <p style="color: #007bff; font-weight: bold;">Processing Ad... Please wait.</p>
        <p style="font-size: 10px; color: #666;">If no popup appears, check your browser settings.</p>
    </div>
    """
    # height=100 kudutha layout-um theryum, script-um background-la trigger aagum
    components.html(ad_script, height=120)
...
if app_mode == "👑 Premium Plan":
    ...
    st.markdown(f'<a href="{upi_url}"><button style="width:100%; height:60px; background:#28a745; color:white; border-radius:12px; font-weight:bold; font-size:18px; border:none;">🚀 Pay ₹99 via GPay</button></a>', unsafe_allow_html=True)

else:
    # Title-ku mela ad trigger logic
    trigger_ads()
    st.title(f"📂 {app_mode}")
